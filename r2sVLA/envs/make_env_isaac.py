...
# Task config
from envs.cfgs.task_cfg import TaskCfg
...
def create_camera():
    ...
    # Ensure quaternion has 4 elements (w, x, y, z)
    cam_orientation_list = list(C["camera_heading_wxyz"])
    if len(cam_orientation_list) != 4:
        raise ValueError(f"camera_heading_wxyz must have 4 elements (w, x, y, z), got {len(cam_orientation_list)}")
    cam_orientation = tuple(cam_orientation_list)
    # Ensure position has 3 elements (x, y, z)
    cam_pos_list = list(C["camera_position"])
    if len(cam_pos_list) != 3:
        raise ValueError(f"camera_position must have 3 elements (x, y, z), got {len(cam_pos_list)}")
    cam_pos = tuple(cam_pos_list)
    # Create intrinsic matrix as a simple list (row-major format)
    # Format: [fx, 0, cx, 0, fy, cy, 0, 0, 1]
    # Using simple Python list to match reference implementation
...
# --------------------------------------------------------------------------------------
def init_scene_from_task_cfg(
    task_cfg: TaskCfg,
    *,
    use_ground_plane: bool = False,
):
    """
    Initialize SceneCtx from a TaskCfg object.
    If robot pose not provided, sample one with robot_placement_candidates_v2().
    """
    # Build cam_dict from task_cfg.camera_info
    cam_dict = {
        "width": task_cfg.camera_info.width,
        "height": task_cfg.camera_info.height,
        "fx": task_cfg.camera_info.fx,
        "fy": task_cfg.camera_info.fy,
        "cx": task_cfg.camera_info.cx,
        "cy": task_cfg.camera_info.cy,
        "camera_heading_wxyz": task_cfg.camera_info.camera_heading_wxyz,
        "camera_position": task_cfg.camera_info.camera_position,
    }
    
    # Sort objects by object_id to ensure consistent ordering
    sorted_objects = sorted(task_cfg.objects, key=lambda obj: obj.object_id)
    ...
    for index, obj_cfg in enumerate(sorted_objects):
        oid_str = str(obj_cfg.object_id)
        oid_to_index[oid_str] = index
        index_to_oid[index] = oid_str
        obj_paths.append(obj_cfg.usd_path)
    
    background_path = task_cfg.background_cfg.background_usd_path

    # Get physics from cfgs (TaskCfg doesn't contain physics info)
    # Priority: args > default
    obj_physics = [DEFAULT_OBJ_PHYSICS for _ in range(len(obj_paths))]
    bg_physics = DEFAULT_BG_PHYSICS

    robot_pos = task_cfg.reference_trajectory[0].robot_pose[:3]
    robot_rot = task_cfg.reference_trajectory[0].robot_pose[3:]

    ground_z = None
    if use_ground_plane:
        try:
            # Use background_point from TaskCfg as ground plane reference
            ground_z = float(task_cfg.background_cfg.background_point[2])
        except (IndexError, TypeError, AttributeError) as e:
            raise ValueError(
                f"use_ground_plane=True but task_cfg.background_cfg.background_point missing/invalid: {e}"
            )
    ...
    assert _SCENE_CTX is not None, "Failed to initialize _SCENE_CTX"
    assert _SCENE_CTX.oid_to_index is not None, "Failed to initialize oid_to_index"
...
# Env factory
# --------------------------------------------------------------------------------------
def _build_manip_env_cfg(scene_cfg_cls, *, num_envs: int, env_spacing: float = 2.5, eval_cfg: Optional[EvaluationConfig] = None):
...
def get_prim_name_from_oid(oid: str | int) -> str:
    ...
    """
    global _SCENE_CTX
    if _SCENE_CTX is None:
        raise RuntimeError(
            "Scene context not initialized. Please call make_env() first, which will "
            "initialize the scene context via init_scene_from_task_cfg(). "
            "This error usually means make_env() was not called, or the module was reloaded."
        )
    oid_str = str(oid)
    if _SCENE_CTX.oid_to_index is None:
        raise ValueError("oid_to_index mapping not available. Scene may not have been initialized properly.")
...
def get_oid_from_prim_name(prim_name: str) -> str:
    ...
    if _SCENE_CTX.index_to_oid is None:
        raise ValueError("index_to_oid mapping not available. Scene may not have been initialized properly.")
...
def make_env(
    ...
    """
    Public entry to construct a ManagerBasedRLEnv from TaskCfg.
    
    Args:
        task_cfg: TaskCfg object loaded from task.json
        cfgs: Optional config dict with physics_cfg, etc.
        num_envs: Number of parallel environments
        device: Device to run simulation on (e.g., "cuda:0")
        bg_simplify: Whether to use simplified ground plane instead of background mesh
    
    Returns: (env, env_cfg)
    """
    from isaaclab.envs import ManagerBasedRLEnv

    # Default cfgs if not provided
    if cfgs is None:
        cfgs = {}

    # Load scene json and initialize global SceneCtx
    init_scene_from_task_cfg(
        task_cfg,
    ...
    ManipEnvCfg = _build_manip_env_cfg(SceneCfg, num_envs=num_envs, env_spacing=2.5, eval_cfg=eval_cfg)
    env_cfg = ManipEnvCfg()
    env_cfg.sim.device = device
    env_cfg.scene.num_envs = num_envs  # double safety
    env_cfg.decimation = eval_cfg.decimation
    env_cfg.sim.dt = 1 / eval_cfg.physics_freq if eval_cfg is not None else 0.01
