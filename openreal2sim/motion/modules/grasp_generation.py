...
def process_one_object(gg_net, obj_idx, obj_dict, proposals_dir, n_points, overwrite, vis_pts_per_gripper=400, min_grasp_score=None, scene_mesh_path=None, object_bbox=None, grasp_point=None, num_grasps=200):
    ...
    Returns:
        ...
        bite_distances: Array of bite distances (N, 2), or None if failed
    '''
    ...
    if npz_path.exists() and not overwrite:
        ...
        grasps, scores, bite_points, bite_distances = load_grasps_npz(npz_path)
        # If bite_points or bite_distances are missing, compute them
        if bite_points is None:
            bite_points, _ = compute_bite_points_from_gripper(grasps, GRIPPER_NAME)
        if bite_distances is None and grasp_point is not None:
            bite_distances = compute_bite_point_to_grasp_point_distances(bite_points, grasp_point)
        return (str(npz_path), grasps, scores, bite_points, bite_distances)
    ...
    if object_bbox is not None:
        ...
        valid_mask = crosses_bbox  # Keep grasps that don't cross bbox
        invalid_mask = ~crosses_bbox  # Grasps that cross bbox
    ...
    # Save valid grasps to NPZ (with bite_points and bite_distances)
    save_grasps_npz(grasps, scores, npz_path, bite_points=bite_points, bite_distances=bite_distances)
    ...
    save_vis_ply(pcs, grasps, scores, vis_path, gripper_name=GRIPPER_NAME, pts_per_gripper=vis_pts_per_gripper, 
                 bite_points=bite_points, filtered_grasps=bbox_filtered_grasps, filtered_bite_points=bbox_filtered_bite_points)
...
def grasp_generation(key=None):
    ...
    for single_key in keys:
        ...
        run_for_key(single_key, key_cfg['n_points'], key_cfg['overwrite'], key_cfg['vis_pts_per_gripper'], key_cfg.get('filter_collisions', True), key_cfg.get('collision_threshold', 0.002), key_cfg.get('min_grasp_score', 0.5), key_cfg.get('keep', 200))
