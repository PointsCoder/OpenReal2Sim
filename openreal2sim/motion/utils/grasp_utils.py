...
def save_grasps_npz(grasps = None, scores = None, save_path = None, bite_points = None, bite_distances = None):
    '''
    ...
    if bite_points is not None:
        save_dict['bite_points'] = bite_points.astype(np.float32)
    if bite_distances is not None:
        save_dict['bite_distances'] = bite_distances.astype(np.float32)
...
def load_grasps_npz(load_path = None):
    ...
    grasps = data['grasps']
    scores = data['scores']
    bite_points = data.get('bite_points', None)
    bite_distances = data.get('bite_distances', None)
    return (grasps, scores, bite_points, bite_distances)
