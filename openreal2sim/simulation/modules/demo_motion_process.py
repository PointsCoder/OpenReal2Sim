...
def recompute_all_traj(scene_json_dict, scene_dict):
    for obj_id, obj in scene_json_dict["objects"].items():
       for key in 'fdpose_trajs', 'simple_trajs', 'hybrid_trajs':
        ...
        new_traj_path = traj_path.replace(".npy", "_recomputed.npy").replace("reconstruction", "simulation").replace("motions","")
...
def demo_motion_process(keys, key_scene_dicts, key_cfgs):
    ...
    for key in keys:
        ...
        manipulated_trajs_path = scene_json_dict["objects"][max_placement_oid][traj_key]
        manipulated_trajs = np.load(manipulated_trajs_path)
        object_masks = build_mask_array(int(max_placement_oid), scene_dict)
        hand_masks = scene_dict["simulation"]["hand_masks"]  
        ...
        #     calib_trajs_path = scene_dict["info"]["objects"][calib_goal_oid]["fdpose_trajs"]
        ...
        scene_json_dict["start_frame_idx"] = downsampled_indices[1]
        ...
        path = base_dir / f"outputs/{key}/simulation/scene_end.glb"
