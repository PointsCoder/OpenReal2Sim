########################################################
## Commandline arguments (IsaacLab only; with --key)
########################################################
import argparse
from isaaclab.app import AppLauncher

parser = argparse.ArgumentParser()
# Add --key argument to process specific key
parser.add_argument("--key", type=str, help="Specific key to process (optional)")
AppLauncher.add_app_launcher_args(parser)
args_isaaclab = parser.parse_args()
# Extract the key argument before AppLauncher processing
key_filter = args_isaaclab.key if hasattr(args_isaaclab, 'key') else None
...
def main():
    ...
    if key_filter:
        if key_filter not in keys:
            raise ValueError(f"Key '{key_filter}' not found in config.yaml keys: {keys}")
        keys = [key_filter]
