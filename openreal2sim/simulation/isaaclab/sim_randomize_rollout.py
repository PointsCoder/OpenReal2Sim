...
from modules.grasp_preprocess.grasp_utils import get_best_grasp_with_hints
...
class RandomizeExecution(BaseSimulator):
    ...
    def inference(self, std: float = 0.0) -> list[int]:
        ...
        if self.record:
            self.save_data(ignore_keys=["segmask", "depth"], env_ids=success_env_ids)
...
def sim_randomize_rollout(keys: list[str], args_cli: argparse.Namespace):
    for key in keys:
        ...
        random_task_cfg_list = randomizer.generate_randomized_scene_cfg(grid_dist=0.0 , grid_num=3, angle_random_range=1/20 * np.pi, angle_random_num=10, traj_randomize_num=10, scene_randomize_num=1, robot_pose_randomize_range=0, robot_pose_randomize_angle=np.pi/180, robot_pose_randomize_num=10)
