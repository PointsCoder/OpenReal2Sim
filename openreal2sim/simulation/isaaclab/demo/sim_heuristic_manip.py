...
from sim_utils_demo.grasp_group_utils import GraspGroup
...
class HeuristicManipulation(BaseSimulator):
    ...
    """
    def __init__(self, sim, scene, sim_cfgs: dict, args, out_dir: Path, img_folder: str, data_dir: Path, num_trials: int, grasp_num: int, robot: str, goal_offset: float, save_interval: int, decimation: int = 1):
    ...
    def follow_object_goals(self, start_joint_pos, sample_step=1, recalibrate_interval = 1, visualize=True):
        ...
        is_success = self.holding_is_success()
    ...
    def holding_is_success(self) -> torch.Tensor:
        ee_w  = self.robot.data.body_state_w[:, self.robot_entity_cfg.body_ids[0], 0:7]
    ...
    def refine_grasp_pose(self, grasp_pos_w: np.ndarray, grasp_quat_w: np.ndarray):
        """
        Refine the grasp pose by the current object state.
        
        Args:
            grasp_pos_w: (B, 3) grasp positions in world frame
            grasp_quat_w: (B, 4) grasp quaternions in world frame (wxyz)
        
        Returns:
            grasp_pos_w: (B, 3) refined grasp positions
            grasp_quat_w: (B, 4) refined grasp quaternions
        """
        current_object_pose = self.object_prim.data.root_state_w[:, :7].cpu().numpy()  # (B, 7)
        pose_offset = current_object_pose[:, :3] - self.scene.env_origins.cpu().numpy()  # (B, 3)
        quat_offset = current_object_pose[:, 3:7]  # (B, 4)
        B = grasp_pos_w.shape[0]
        
        # Make copies to avoid modifying input arrays
        grasp_pos_w = grasp_pos_w.copy()  # (B, 3)
        grasp_quat_w = grasp_quat_w.copy()  # (B, 4)
        
        for b in range(B):
            # Add object pose offset to grasp position
            grasp_pos_w[b, :3] = pose_offset[b, :3] + grasp_pos_w[b, :3]
            # Compose rotations: obj_rot @ grasp_rot
            grasp_mat_w = transforms3d.quaternions.quat2mat(grasp_quat_w[b])
            obj_mat = transforms3d.quaternions.quat2mat(quat_offset[b])
            grasp_mat_w = obj_mat @ grasp_mat_w
            grasp_quat_w[b] = transforms3d.quaternions.mat2quat(grasp_mat_w)
    ...
    def grasp_trials(self, gg):
        ...
        for start in range(self.grasp_round, len(idx_all), B):
            ...
            delta_batch    = rng.normal(-0.002, self.std, size=(B,)).astype(np.float32)
    ...
    def inference(self, std: float = 0.0) -> list[int]:
        ...
        while success_num < self.grasp_num and self.trial_num < self.num_trials:
            ...
            if self.grasp_idx >= 0:
                ...
                rets = self.grasp_trials(gg)

            print("[INFO] Re-exec all envs with the winning grasp, then follow object goals.")
            if (rets is None or rets[0]["success"] == False) and self.grasp_idx >= 0:
                print("[ERR] no proposal succeeded to lift after full search.")
                return 0
            ...
            for i in range(0, len(rets), B):
                ...
                if len(cleaned_success_ids) > 0:
                    self.save_data(ignore_keys=["segmask", "depth"], env_ids=cleaned_success_ids, export_hdf5=True)
...
def sim_heuristic_manip(key: str, args_cli: argparse.Namespace, config_path: Optional[str] = None, config_dict: Optional[dict] = None):
    ...
    my_sim = HeuristicManipulation(
        ...
        data_dir = data_dir, num_trials = num_trials, grasp_num = grasp_num, robot = robot, goal_offset = goal_offset, save_interval = save_interval, decimation=decimation)
