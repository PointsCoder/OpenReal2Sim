...
class HeuristicManipulation(BaseSimulator):
    ...
    """
    def __init__(self, sim, scene, sim_cfgs: dict, args, out_dir: Path, img_folder: str, data_dir: Path, num_trials: int, grasp_num: int, robot: str):
        ...
        while success_num < self.grasp_num and self.trial_num < self.num_trials:
            ...
            for i in range(0, len(rets), B):
                ...
                if len(cleaned_success_ids) > 0:
                    self.save_data(ignore_keys=["segmask", "depth"], env_ids=cleaned_success_ids, export_hdf5=False)
                    for k in cleaned_success_ids:
                        ...
                        end_object_pose[:3] = end_object_pose[:3] - self.scene.env_origins.cpu().numpy()[k]
...
def sim_heuristic_manip(key: str, args_cli: argparse.Namespace, config_path: Optional[str] = None, config_dict: Optional[dict] = None):
    ...
    my_sim = HeuristicManipulation(
        ...
        data_dir = data_dir, num_trials = num_trials, grasp_num = grasp_num, robot = robot )
