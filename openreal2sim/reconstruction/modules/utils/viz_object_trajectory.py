...
def visualize_world_trajs_all(
    ...
    *,
    ...
    for oid, obj in objs.items():
        ...
        assert trajs_path.exists(), f"[{key}] trajs '{traj_key}' not found: {trajs_path}"
    ...
    pc = trimesh.points.PointCloud(vertices=pts_all, colors=cols_all)
    pc.export(str(ply_path))
    print(f"[{key}][PLY] aggregated: {ply_path}  (pts={len(pts_all):,})")
