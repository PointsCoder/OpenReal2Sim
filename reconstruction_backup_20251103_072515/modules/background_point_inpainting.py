...
def hybrid_fill(depth0: np.ndarray, fg_img: np.ndarray, bg_img: np.ndarray, K: np.ndarray, 
                           ...
                           plane_mask: np.ndarray = None) -> np.ndarray:
    """
    ...
    plane_masks = []
    for plane_idx in range(1, plane_mask.max()):
...
def background_point_inpainting(keys, key_scene_dicts, key_cfgs):
    ...
    for key in tqdm.tqdm(keys):
        ...
        plane_mask = scene_dict["recon"]["plane_mask"] if scene_dict["recon"]["plane_mask"] is not None else None
        ...
        elif completion_mode == "hybrid" and plane_mask is not None:
            depth_bg = hybrid_fill(depth0, fg_img, bg_img, K, object_mask, device, plane_mask)
        else:
